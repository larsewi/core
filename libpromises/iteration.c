 ...
 *
 * How does it work? It looks for e.g. a '.' in both #iterctx->pp->promiser and
                              ...
                              EvalContext *evalctx, char *varname)
{
    ...
    unexp_pos = iterctx->pp->promiser;
    while (!done && (unexp_pos = strchr(unexp_pos, CF_NS)) != NULL)
    {
        char *exp_pos = (char *)varname;
        while (!done && (exp_pos = strchr(exp_pos, CF_NS)) != NULL)
        {
            char tmp = *exp_pos;
            *exp_pos = '\0';
            if (StringSetContains(scopes, varname))
            {
                *exp_pos = CF_MANGLED_NS;
                *unexp_pos = CF_MANGLED_NS;
                done = true;
            }
            else
            {
